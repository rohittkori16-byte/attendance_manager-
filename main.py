...
def main():
    ...
    if not mongo_url:
        print(
            "[ERROR] MONGO_URI is missing from the .env file."
        )
        return
    ...
    manager.student_collection.delete_many({
        "roll_no": {
            "$in": [roll1, roll2]
        }
    })

    manager.attendance_collection.delete_many({
        "roll_no": {
            "$in": [roll1, roll2]
        }
    })
    ...
    records = manager.display_attendance()
